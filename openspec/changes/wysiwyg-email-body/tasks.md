...
- [x] 2.1 Реализовать `src/Service/CampaignTokenFiller.php` (HTML-экранирование `ENT_QUOTES | ENT_SUBSTITUTE` для тела и прехедера, plain для темы); проверить `php bin/phpunit --filter CampaignTokenFillerTest` — тесты на `<` в названии организации и на токен внутри атрибута
...
- [x] 2.3 Перевести `MailingService::sendEmail()` на `TemplatedEmail` через `CampaignEmailRenderer`; проверить обновлённый `MailingServiceTest` — письмо содержит html и text, токены экранированы, вложения и TO/CC не изменились
