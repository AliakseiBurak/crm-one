...
См. `proposal.md` — Why. Историческая база: состояние до change, важное для понимания подхода (ссылки на строки относятся к версии до внедрения):
...
- Standalone-разметка обоих публичных результатов отписки без интерфейса CRM, внешних ассетов и ссылок на приложение.
...
TipTap по умолчанию выбрасывает `style` и незнакомые узлы. Поэтому allowlist санитайзера и схема редактора проектируются как один email-safe контракт:

- глобальный атрибут `style` на всех разрешённых узлах и марках (`strong`/`b`, `em`/`i`, `u`, `s`/`del`, inline `code`, `span`, абзацы, заголовки H1–H6, списки, цитата, `hr`); тулбар предоставляет H1, H2 и H3;
- стилизованный блочный `div` с содержимым `block+`;
- изображение `img` с `src|alt|title|width|height|style`, только внешний `https` URL;
- ссылка `a` с `href|target|rel|style` — именно эти атрибуты сериализует TipTap Link;
- стилизованная таблица с одним каноническим `tbody`, стилизованными `tr/td/th`, `colspan` и `rowspan`; служебные `colwidth` и класс ссылки в схеме не регистрируются.

`pre`/code-block, `thead`, `tfoot` и устаревшие presentation-атрибуты таблиц (`border`, `cellpadding`, `cellspacing`, `align`, `valign`, `bgcolor`, атрибуты `width`/`height` таблиц и ячеек) в allowlist не входят: code-block не нужен продукту, ProseMirror-таблица TipTap 2 требует строки прямыми потомками и не имеет идиоматичной модели секций, а email-совместимость этих свойств задаётся стилями и статичным шеллом письма. Расширение allowlist обязано сопровождаться поддержкой в схеме; один консолидированный e2e-тест round-trip фиксирует контракт. Гарантия каноническая, а не побайтовая: синонимы `b → strong`, `i → em`, `del → s`, порядок атрибутов, CSS-значения и написание HTML-сущностей нормализуются, содержимое не теряется.

### D4. Санитизация: symfony/html-sanitizer + свой `style`-санитайзер

Альтернатива — HTMLPurifier (`mews/purifier`): готовый CSS-allowlist, но свой конфиг-язык, обязательная writable-кеш-директория, агрессивная нормализация и менее удобный per-attribute API. Выбран Symfony-компонент: нативная интеграция с Twig/DI, `AttributeSanitizerInterface` для CSS-allowlist, предсказуемость при контролируемом выводе редактора. Лимиты: форма — 200 000 символов (валидируется до санитизации), `max_input_length` санитайзера — 262 144 байта и в рамках этого change не меняется. `style`-санитайзер отбрасывает объявление, если его значение содержит токен `{{...}}`: токены допустимы в тексте и HTML-атрибутах, но не в CSS, где подстановка могла бы добавить свойства после санитизации; остальные объявления того же `style` сохраняются. Пустой после санитизации результат — ошибка валидации.
...
`CampaignTokenFiller` подставляет значения с `htmlspecialchars(ENT_QUOTES | ENT_SUBSTITUTE, UTF-8)` для тела; прехедер заполняется plain-значениями, включая `{{unsubscribe_url}}`, и экранируется Twig-автоэкранированием ровно один раз; тема заполняется без HTML-экранирования. Альтернатива «рендерить тело как Twig-шаблон» отвергнута: тело — пользовательский HTML, а не шаблон.
...
Миграции нет (колонка уже `LONGTEXT`); фикстуры переписываются на HTML-тела. e2e получают хелпер `setCampaignBody()`: переключить режим исходного HTML → заполнить textarea → вернуть визуальный режим. Общие хелперы `deleteCampaign()` и `deleteGroup()` удаляют созданные данные в `finally`, поэтому общая БД фикстур не меняется между прогонами; тест, мутировавший общую failed-фикстуру, удалён. Библиотечный выбор (TipTap) и решения D3/D4/D5/D10 фиксируются в ADR-0014.

### D10. Страницы отписки — общий standalone-шаблон

`/unsubscribe/{token}` публично доступен из писем в интернете и выбирает один из двух результатов: `confirmed` после первой успешной отписки или `already` при повторном переходе. Оба шаблона наследуют общий `unsubscribe/layout.html.twig` и не наследуют `base.html.twig`: инлайн-стили, без шапки, подвала, навигации, `<link>`, `<script>` и каких-либо ссылок на приложение, с `robots: noindex, nofollow`. Каждый документ содержит только заголовок и поясняющий текст своего результата. Цель — не раскрывать маршруты и оформление внутреннего инструмента снаружи.
...
flowchart LR
    ...
    subgraph App[Symfony-приложение]
        ...
        Mail[MailingService<br/>Email (html + text) → SMTP]
    end
...
sequenceDiagram
    ...
    alt превышен лимит
        ...
        C->>S: sanitize(body)
        S->>S: allowlist элементов, атрибутов и URL
        S->>S: allowlist CSS; объявления с {{...}} удаляются
        S-->>C: очищенный HTML
...
sequenceDiagram
    ...
    M->>SMTP: Email (html + text + вложения)
    alt успех
...
- [Round-trip «исходник ↔ визуальный режим» теряет неизвестные атрибуты] → инвариант D3, глобальный `style` на узлах и марках, поддержка `div`, H1–H6, image height и email-table serializer; один консолидированный e2e-тест проверяет каждый разрешённый элемент/атрибут и идемпотентность второго round-trip.
...
- [Санитайзер усекает по `max_input_length` в байтах, а форма ограничивает 200 000 символов] → конфигурация приложения в этом change не меняется; до production-выкладки лимит multibyte-тел решается отдельным серверным изменением (request/body limit), а не правкой `html_sanitizer.yaml`.
- [Токен в CSS-значении добавляет свойства после санитизации] → `CampaignStyleAttributeSanitizer` отбрасывает объявление с `{{...}}`, остальные объявления сохраняются; тест фиксирует контракт.
...
2. Добавить конфиг санитайзера, сервисы, шаблон письма, маршруты предпросмотра, редактор; переключить `MailingService` на рендерер; перевести оба результата отписки на `unsubscribe/layout.html.twig`.
3. Переписать фикстуры на HTML-тела. Перед выкладкой данные кампаний пересоздаются во всех средах, включая production: миграции схемы и данных нет, старые plain-text тела не переносятся.
4. Обновить e2e и функциональные тесты.
...
4. Серверный лимит для multibyte-тел (200 000 символов против 262 144 байт санитайзера) — отдельное инфраструктурное изменение; спецификация и `html_sanitizer.yaml` этого change не меняют.
