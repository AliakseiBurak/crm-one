...
### D10. Страница отписки — standalone-шаблон

`/unsubscribe/{token}` публично доступен из писем в интернете, поэтому `unsubscribe/{confirmed,already}.html.twig` не наследуют `base.html.twig`: общий `unsubscribe/layout.html.twig` с инлайн-стилями, без шапки, подвала, `<link>`, `<script>` и каких-либо ссылок на приложение, с `robots: noindex, nofollow`. Цель — не раскрывать маршруты и оформление внутреннего инструмента снаружи.
