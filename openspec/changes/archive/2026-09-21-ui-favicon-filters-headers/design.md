...
- Переименование `<title>` вкладки дашборда (остаётся «Панель») — не входит в scope
- Исключение hero-заголовка главной из правила `h1` — не входит в scope
...
- `<title>Панель</title>` в `dashboard.html.twig` остаётся: требование дельты касается видимого заголовка `h1`, title вкладки браузера вне scope (Non-Goal)
- Правило `h1` действует для всех `h1`, включая `.hero__title` главной страницы: полоса и `padding-left` появляются и у hero-заголовка; его цвет (`#22505f`) и размер сохраняются за счёт специфичности класса `.hero__title`. Принято по действующей спеке `web-interface` (сценарий «Заголовок первого уровня с синей полосой»)
...
- `templates/home/index.html.twig`:
  ...
  - `Ожидают` → `Ожидают звонка`; подписи: `Сегодня`, `За 7 дней`, `За 30 дней`
...
### Decision 9: Синхронизация спек и чистка статического анализа
- Дельта `dashboard` дополняется MODIFIED-блоками для требований, чьи сценарии ссылались на старые подписи: `Статистика просроченных звонков` и `Исключающая логика waiting-категорий` (текст шагов переведён на «секция + период»); имена сценариев сохранены, т.к. валидатор запрещает их переименование внутри MODIFIED
- В дельте `web-interface` пример в сценарии «Крупные числа статистики» приведён к новым секциям
- `composer stan` (level 8) даёт 5 ошибок в файлах вне диффа; они исправляются здесь, чтобы архив не наследовал красный статический анализ:
  - `UserController:67,77` — мёртвые null-сравнения после `(string)`-присваиваний
  - `OrganizationHideController:112` — `User::$email` nullable: null-safe сбор списка email
  - `MailingService:399` — администратор без email пропускается с предупреждением в лог
  - `phpstan-baseline.neon` — удалены 2 устаревшие записи (`UserController` `identical.alwaysFalse`, `CallRepository` `offsetAccess.nonOffsetAccessible`)
- Проверки: `composer cs:check` — чисто, `composer stan` — 0 ошибок, `php bin/phpunit` — зелёный, `npm run build` + прогон e2e
...
9. Синхронизация дельт: MODIFIED для `Статистика просроченных звонков` и `Исключающая логика waiting-категорий`, пример в `web-interface`
10. Чистка PHPStan: 5 ошибок + 2 устаревшие baseline-записи
